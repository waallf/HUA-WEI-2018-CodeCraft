...
class Matrix:
    ...
    def numberSubMatrix(self,number):
        m = Matrix(self.nrows, self.ncols)
        for row, col, element in self.elements():
            #print(str(row)+" "+str(col)+" "+str(element)+" "+str(number))
    ...
    ##
    ## 获得该矩阵的子矩阵
    ## row1行到row2行 col1列到col2列
    ## row1 row2 col1 col2 全部包含
    ## row1 row2 col1 col2 是从1开始的
    #############################################################
